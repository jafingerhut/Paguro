...
public abstract class RrbTree8<E> implements BaseList<E>, Indented {
     ...
     * available there. */
    public static class MutableRrbt<E> extends RrbTree8<E> implements MutableList<E> {
        private E[] focus;
...
//        }
         ...
         */
        @SuppressWarnings("unchecked")
        public RrbTree8<E> join(RrbTree8<E> that) {
...
//        split.right().debugValidate();
     ...
     * is available there. */
    public static class ImRrbt<E> extends RrbTree8<E> implements ImList<E>, Serializable {
        ...
        private static class SerializationProxy<E> implements Serializable {
            ...
            private final int size;
            private transient RrbTree8<E> rrbTree;
            SerializationProxy(RrbTree8<E> v) {
                size = v.size();
                rrbTree = v;
            }
...
//        split.right().debugValidate();
    ...
    /** {@inheritDoc} */
    @Override abstract public RrbTree8<E> append(E t);
     ...
     */
    @SuppressWarnings("WeakerAccess")
    public abstract RrbTree8<E> insert(int idx, E element);
...
I'm thinking the signature is something like:
    ...
    public RrbTree8<E> join(RrbTree8<? extends E> that)
...
*/
     ...
     */
    public abstract RrbTree8<E> join(RrbTree8<E> that);

    /** Internal method - do not use. */
    abstract Node<E> pushFocus();

    /** {@inheritDoc} */
    @Override abstract public RrbTree8<E> replace(int index, E item);
    ...
    /**
     ...
     @return two new sub-trees as determined by the split point.  If
             ...
             never null).
     */
    abstract public Tuple2<? extends RrbTree8<E>,? extends RrbTree8<E>> split(int splitIndex);

    /**
     Returns a new RrbTree minus the given item (all items to the
     right are shifted left one).  This is O(log n).
     */
    public RrbTree8<E> without(int index) {
        if ( (index > 0) && (index < size() - 1) ) {
            Tuple2<? extends RrbTree8<E>,? extends RrbTree8<E>> s1 = split(index);
            Tuple2<? extends RrbTree8<E>,? extends RrbTree8<E>> s2 = s1._2().split(1);
